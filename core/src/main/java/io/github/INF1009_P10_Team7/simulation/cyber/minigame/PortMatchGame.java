...
public class PortMatchGame implements IMiniGame {
    ...
    public void render(ShapeRenderer sr, SpriteBatch batch, BitmapFont ignoredFont) {
        ...
        sr.rect(wx + 40, wy + 180, 300, 300);
        // Service column bg
        sr.rect(wx + 620, wy + 180, 300, 300);

        // Row highlights
        float rowH  = 56f;
        float rowY0 = wy + 180 + 240;  // top row bottom
        for (int i = 0; i < 5; i++) {
        ...
        medFont.draw(batch, "  [ NETWORK CONFIG // PORT MAPPER ]    [TAB=CLOSE]",
            wx + 10, wy + wh - 13f);

        // Column headers
        medFont.setColor(new Color(0.7f, 0.4f, 0f, 1f));
        medFont.draw(batch, "PORTS", wx + 155, wy + 498f);
        medFont.draw(batch, "SERVICES", wx + 718, wy + 498f);

        // Key hint top
        smallFont.setColor(0.45f, 0.45f, 0.45f, 1f);
        medFont.draw(batch, "Instructions:  Press a port number ( 1 – 5 ) to select it,",
            wx + 90, wy + wh - 75f);
        medFont.draw(batch, "then press a service letter ( A – E ) to make the match.",
            wx + 90, wy + wh - 96f);

        // Port and service rows
        float rowY0f = wy + 180 + 240;
        for (int i = 0; i < 5; i++) {
            ...
            if (pDone) {
                ...
                medFont.draw(batch, "→ " + SERVICES[portMatched[i]] + " ✓", wx + 170, ry);
            }
        ...
        if (selectedPort < 0) {
            ...
            medFont.draw(batch, "Press a port number:  1  2  3  4  5", wx + 280, wy + 155f);
        } else {
        ...
        if (solved) {
            ...
            medFont.draw(batch, "✓  ALL PORTS MAPPED — KEY ACQUIRED  (closing...)", wx + 180, wy + 48f);
        } else if (wrongFlash > 0) {
            medFont.setColor(1f, 0.2f, 0.2f, 1f);
            medFont.draw(batch, "✗  " + wrongMsg, wx + 280, wy + 48f);
        }
