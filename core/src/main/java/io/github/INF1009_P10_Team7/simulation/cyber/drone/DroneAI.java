 ...
 * Rendering: detailed mechanical drone with layered glow effects.
 */
public class DroneAI {

    private final Vector2 position;
    private float facingAngle = 0f;
    private final float radius     = 14f;

    private final float patrolSpeed = 60f;
    private final float chaseSpeed  = 115f;
    private final float sightRange  = 180f;
    private final float sightAngle  = 120f;
    ...
    // =========================================================================
    // RENDER — multi-layered cyberpunk drone
    // =========================================================================
    ...
    public void render(ShapeRenderer sr) {
        ...
        // soft gradient cone — outer faint layer (kept very transparent)
        sr.setColor(chasing ? new Color(0.9f, 0.1f, 0f, 0.04f * alertLevel)
                            : new Color(0.9f, 0.85f, 0f, 0.025f));
        int steps = 24;
        ...
        // inner brighter cone (half range) — slightly more visible but still subtle
        float innerRange = sightRange * 0.5f;
        sr.setColor(chasing ? new Color(1f, 0.2f, 0f, 0.08f * alertLevel)
                            : new Color(1f, 1f, 0.1f, 0.045f));
        for (int i=0;i<steps;i++) {
            float a1 = r1 + (r2-r1)*i/steps;
            float a2 = r1 + (r2-r1)*(i+1)/steps;
            sr.triangle(ax, ay,
        ...
        for (int i=0;i<4;i++) {
            ...
            // draw a thin rectangle for the arm
