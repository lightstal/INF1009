...
public class CyberHudRenderer {
    ...
    public CyberHudRenderer(ShapeRenderer sr, SpriteBatch batch,
    ...
    public void renderObjectiveBanner(float stateTime, float bannerTimer,
        ...
        float fadeIn  = Math.min(1f, (Math.max(0f, 1.8f - bannerTimer
            + (bannerTimer > 1.8f ? bannerTimer - 1.8f : 0f))) / 0.35f);
    ...
    public void renderMinimap(TransformComponent tc, boolean[][] wallGrid,
        ...
        if (wallGrid != null) {
            for (int row = 0; row < TileMap.ROWS; row++) {
                for (int col = 0; col < TileMap.COLS; col++) {
                    sr.setColor(wallGrid[row][col]
                        ? new Color(0.2f, 0.25f, 0.3f, 0.8f)
                        : new Color(0.05f, 0.08f, 0.1f, 0.5f));
        ...
        sr.setColor(exitUnlocked ? new Color(0.8f, 0f, 1f, 1f) : new Color(0.3f, 0.05f, 0.05f, 0.8f));
