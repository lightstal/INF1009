...
public class CaesarCipherGame implements IMiniGame {
    ...
    private void buildFonts() {
        disposeFonts();
        bigFont   = new BitmapFont(); bigFont.getData().setScale(1.6f);   bigFont.setUseIntegerPositions(true);
    ...
    public void render(ShapeRenderer sr, SpriteBatch batch, BitmapFont ignored) {
        ...
        medFont.draw(batch, "  [ CRYPTOGRAPHY // CAESAR CIPHER DECODER ]    [TAB = CLOSE]",
            WX + 10, WY + WH - 12f);
        ...
        smallFont.draw(batch,
            "[ A / ◄ ]  rotate left          [ D / ► ]  rotate right          [ ENTER ]  submit",
        ...
        if (solved) {
            ...
            layout.setText(medFont, "✓  CIPHER BROKEN — KEY ACQUIRED  (closing...)");
            medFont.draw(batch, "✓  CIPHER BROKEN — KEY ACQUIRED  (closing...)",
                WX + WW/2f - layout.width/2f, WY + 110f);
        } else if (wrongFlash > 0) {
            medFont.setColor(1f, 0.2f, 0.2f, 1f);
            layout.setText(medFont, "✗  WRONG SHIFT — TRY AGAIN");
            medFont.draw(batch, "✗  WRONG SHIFT — TRY AGAIN",
                WX + WW/2f - layout.width/2f, WY + 110f);
        } else {
